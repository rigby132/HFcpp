...
hf::Matrix hf::HFSolver::calcDensity(const Matrix& coeff)
{
	...
	density.setZero();
	for (size_t t = 0; t < m_basisSize; t++)
		for (size_t u = 0; u < m_basisSize; u++)
...
hf::Matrix hf::HFSolver::calcOverlap()
{
	...
	for (size_t r = 0; r < m_basisSize; r++)
		for (size_t s = 0; s < m_basisSize; s++) {
...
template <typename FLOAT = double>
std::vector<FLOAT> calcPotentialIntegral(const std::vector<FLOAT>& start, const size_t qa,
	const size_t qb, const FLOAT x_pa, const FLOAT x_pz, const FLOAT x_ab, const FLOAT p)
{
	assert(start.size() > 0);

	const size_t N = start.size() - 1;
	const size_t qn = qa + qb; // Calculate up to i+1
	...
	for (size_t j = 1; j <= qb; j++)
		for (size_t i = qa; i <= qn - j; i++)
			for (size_t n = 0; n <= N - qn; n++)
...
std::vector<FLOAT> calcSingleRepulsionIntegral(const std::vector<FLOAT>& start,
	const size_t(&quantum)[4], const FLOAT(&expo)[4], const FLOAT(&cx)[4])
{
	...
	for (size_t i = 1; i <= qi + qj + qk + ql; i++) {
...
hf::Repulsions hf::HFSolver::calcRepulsionIntegrals()
{
	...
	for (auto& lvl1 : integrals) {
		...
		for (auto& lvl2 : lvl1) {
			...
			for (auto& lvl3 : lvl2)
				lvl3.resize(m_basisSize, nan);
		}
	}

#pragma omp parallel for schedule(guided)
	for (long long r = 0; r < m_basisSize; r++)
		for (long long s = 0; s < m_basisSize; s++)
			for (long long t = 0; t < m_basisSize; t++)
				for (long long u = 0; u < m_basisSize; u++) {
					const auto& cg0 = m_basis[r];
					const auto& cg1 = m_basis[s];
						...
						for (const auto& g1 : cg1.gtos_)
							for (const auto& g2 : cg2.gtos_)
								for (const auto& g3 : cg3.gtos_) {
									size_t qsum = g0.i_ + g1.i_ + g2.i_ + g3.i_;
...
double hf::HFSolver::solve(double tolerance)
{
	...
	for (size_t i = 0; i < m_basisSize; i++)
	...
	do {
		...
		i++;
	} while (maxDelta > tolerance && i < 100);
